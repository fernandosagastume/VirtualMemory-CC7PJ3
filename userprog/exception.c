...
//----------------------------- Our Solution ------------------------------------------//
  ...
  struct sup_page_table_entry* SPTE  = get_SPTE(upage, &curr->SPT);
  //If the page not present, we allocate a new page
  if (not_present){
    if(SPTE){
      //Funciones para hacer loading en caso de reclamation
      if(!SPTE->page_loaded){
    ...
    if(fault_addr >= (f->esp - 32) && fault_addr < PHYS_BASE && !SPTE){
      ...
      success = install_page (upage, kpage, writable); //Growth Stack
      if(!success){
